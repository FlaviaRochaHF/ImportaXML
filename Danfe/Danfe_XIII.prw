...
If MsgYesNo( 'Deseja imprimir em lote ?', 'Impressão Xml' )
	...
	If ParamBox(aPergs, "Parametros de impressão em lote")
		...
		MemoWrite("C:\TEMP\danfe_xiii.sql" , cQuery) //FR - 01/12/2020
		//FR - 01/12/2020
		
		dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAliasZBZ)

		DbSelectArea(cAliasZBZ)
		(cAliasZBZ)->(Dbgotop())		//FR - 01/12/2020
		While !(cAliasZBZ)->(Eof())
...
if Type( "oNF:_InfNfe:_Cobr:_Dup" ) <> "U"
	nFaturas := IIf(Type("oNF:_INFNFE:_COBR:_DUP")=="U",0,IIf(oFatura<>Nil,IIf(ValType(oNF:_InfNfe:_Cobr:_Dup)=="A",Len(oNF:_InfNfe:_Cobr:_Dup),1),0))
else
	nFaturas := 0
endif
...
Else
	oDanfe:Say(nLine+172,612,"CPF",oFont08N:oFont)
	oDanfe:Say(nLine+180,612,TransForm(oEmitente:_CPF:TEXT,IIf(Len(oEmitente:_CPF:TEXT)<>14,"@r 999.999.999-99","@r 99.999.999/9999-99")),oFont08:oFont) 
EndIf
