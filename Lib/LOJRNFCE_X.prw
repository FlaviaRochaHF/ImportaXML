...
//Modalidades de TEF disponÌveis no sistema
...
#DEFINE DLL_SIGALOJA			0			// Usa somente perifÈricos da SIGALOJA.DLL
#DEFINE DLL_SIGALOJA_AUTOCOM	1			// Usa perifÈricos da SIGALOJA.DLL e da AUTOCOM
#DEFINE DLL_AUTOCOM				2			// Usa somente perifÈricos da AUTOCOM
...
// 		- ao adicionar uma tag aqui inserir na funÁıes da sigaloja, 
//		totvsapi e autocom para tratar as tags por modelo de ECF, 
//		nos fontes dos modelos e no LOJA1305 que trata da remoÁ„o da tag nao utilizada
//**************************************************************************************************//
...
#DEFINE TAG_TABS_INI		"<tb>"	//tabulaÁ„o
...
#DEFINE TAG_NIVELQRCD_INI "<correcao>" // nivel de correÁ„o do QRCode
...
#DEFINE MTAG_TABS_INI		TAG_ESC+"B"	//tabulaÁ„o
...
#DEFINE MTAG_NIVELQRCD_INI "" // nivel de correÁ„o do QRCode
...
User Function HFImpNFCE(	oNFCe1		, oProt1	, nDecimais	, aFormas	,; 
	...
	Local nTotDesc 		:= 0
	Local nTotAcresc	:= 0 		
	Local nFtQrCode		:= 0.38 //Fator Conersao tamanho QrCode, referente a posiÁ„o da linha 1
	...
	*/
	...
	If lContigen
		...
		nAuxLn += 40
		oPrint:Say( nAuxLn,1, PadC("Pendente de autorizaÁ„o", 85), oFont )
	EndIf

	nAuxLn += 60	//quebra de linha entre as divisoes

	/*
		Divis„o II ñ InformaÁıes de Detalhe da Venda
		* a impressao dessa divis„o È opcional ou conforme definido por UF
	*/
	...
	/*
		Divis„o III ñ InformaÁıes de Total do DANFE NFC-e
	*/
	...
	nAuxLn += 40
	//se existir ISSQN, o VALOR TOTAL È igual a soma da tag vProd + vServ
	...
	/*
		DIVISAO IV ñ InformaÁıes da consulta via chave de acesso		
	*/
	...
	/*
		DIVISAO VI ñ InformaÁıes sobre o Consumidor
	*/	
	If Empty(aDestNfce)
	...
	Else
		...
		/*
		OPCIONALMENTE poder· ser incluÌda nesta divis„o tambÈm o nome do consumidor e/ou seu endereÁo.
		No caso de emiss„o de NFC-e com entrega em domicÌlio È OBRIGAT”RIA a impress„o do nome do consumidor e do endereÁo de entrega.
		*/
	...
	/*
		DIVISAO VII ñ InformaÁıes de IdentificaÁ„o da NFC-e e do Protocolo de AutorizaÁ„o
		N˙mero SÈrie Emiss„o DD/MM/AAAA hh:mm:ss
	*/
	aDtHrLocal := LjUTCtoLoc(aIdNfce:_DHEMI:TEXT)
	
	cTextoAux := "NFC-e n∞" + aIdNfce:_NNF:TEXT + " "
	cTextoAux += "SÈrie " + aIdNfce:_SERIE:TEXT + " "
	...
	If !lContigen
		...
		/* Protocolo de AutorizaÁ„o  DD/MM/AAAA hh:mm:ss */
		cTextoAux := "Protocolo de AutorizaÁ„o: " + cProtAuto + " "
		oPrint:Say( nAuxLn, 1, PadC(cTextoAux,85), oFont )
		nAuxLn += 40

		cTextoAux := "Data de AutorizaÁ„o: "
		cTextoAux += PadL( Day(aDtHrLocal[1]),2,"0" ) + "/"			//DD
		cTextoAux += PadL( Month(aDtHrLocal[1]),2,"0" ) + "/"		//MM
		cTextoAux += cValToChar( Year(aDtHrLocal[1]) ) + " "		//AAAA
	...
	/*
		Divis„o VIII ñ ¡rea de Mensagem Fiscal
	*/	
	...
	If cAmbiente == "2"		
		oPrint:Say( nAuxLn, 1, PadC("EMITIDA EM AMBIENTE DE HOMOLOGA«√O ñ SEM VALOR FISCAL",85), oFont )
		nAuxLn += 60
	EndIf
	...
	/*
		DIVISAO V ñ InformaÁıes da Consulta via QR Code
		A imagem do QR Code poder· ser CENTRALIZADA (conforme o rdmake) ou
		impressa ‡ esquerda das informaÁıes exigidas nas Divisıes VI e VII
	*/
	// obtem o QR-Code
	cKeyQRCode := U_HFLjQrCo(oNFCe, cAmbiente)		
	
	/* Tratamento feito para controlar posiÁ„o da impressao do QRCODE, pois o metodo
	do mesmo, nao esta respeitando a posiÁ„o de impressao quando a Quebra de Pagina */
	...
	/*		
		DIVISAO IX ñ Mensagem de Interesse do Contribuinte
	*/
...
Local nIniDesc		:= 1	//indica a posiÁ„o inicial da leitura da tag xProd (descriÁ„o do produto)
Local nFimDesc		:= 0
Local nCodDesc		:= 0	//soma das colunas Codigo + " " + Descricao
Local lImpDesc		:= .T.	//variavel de controle que verifica se havera mais linhas para impressao da DescriÁ„o
Local aColDiv2		:= {}	//largura das colunas da Divisao II

//Parametros enviados pela funÁ„o no fonte LOJNFCE
Default cXml 		:= ""
Default cXmlProt	:= ""
Default cChvNFCe	:= ""
Default lDanfePad	:= .F.
...
BEGIN SEQUENCE
	...
	If lDanfePad .AND. lLJ7084
		...
		If !lImpComum .AND. !lPos .AND. nHdlECF == -1		
			...
			If nHdlECF == -1
				If !IsBlind()
					MsgStop("NFC-e: N„o foi possÌvel estabelecer comunicaÁ„o com a Impressora:" + cImpressora)
				Else
					conout("NFC-e: N„o foi possÌvel estabelecer comunicaÁ„o com a Impressora:" + cImpressora)
		...
		//Valida se existe nDecimais, variavel È Privete declarada no Loja701
		If Type("nDecimais") == "U"
		...
		//Ambiente (Normal ou HomologaÁ„o)
		cAmbiente := oNFCe:_NFE:_INFNFE:_IDE:_TPAMB:TEXT
		...
		If !aIdNfce:_TPIMP:TEXT $ "45"
			If !IsBlind()
				MsgStop("Nfc-e: Tipo de Impress„o incompatÌvel: "+aIdNfce:_TPIMP:TEXT)
			Else
				Conout("Nfc-e: Tipo de Impress„o incompatÌvel: "+aIdNfce:_TPIMP:TEXT)
			EndIf			
		...
		/*
			DIVISAO I - InformaÁıes do CabeÁalho
		*/
		/* Logotipo da empresa - (utilizar a ferramenta da prÛpria fabricante) */
		...
		/* EndereÁo Completo, nro, bairro, MunicÌpio - UF */
		cTextoAux := AllTrim(aEmitNfce:_ENDEREMIT:_XLGR:TEXT) 	+ ","
		cTextoAux += AllTrim(aEmitNfce:_ENDEREMIT:_NRO:TEXT)	+ ","
		cTextoAux += AllTrim(aEmitNfce:_ENDEREMIT:_XBAIRRO:TEXT)+ ","
		cTextoAux += AllTrim(aEmitNfce:_ENDEREMIT:_XMUN:TEXT)	+ ","
		...
		If lContigen
			...
			cTexto += cCRLF
			cTexto += (cTagCondIni + "Pendente de autorizaÁ„o" + cTagCondFim)
			cTexto += TAG_CENTER_FIM
			cTexto += Replicate(cCRLF,2)
		EndIf

		/*
			DIVISAO II ñ InformaÁıes de detalhes de produtos/serviÁos
			A impressao dessa divis„o È opcional ou conforme definido por UF
		*/
		//
		...
		For nX := 1 to Len(aItemNfce)
			...
			lImpDesc := .T.
			nIniDesc := 1
			nFimDesc := aColDiv2[2] + aColDiv2[3] + aColDiv2[4] + aColDiv2[5] + aColDiv2[6] + 4 //4 espaÁos separadores
			...
			//Tratamento necess·rio pois dependendo tamanho das informaÁıes dos itens a serem impressos,
			//apos um determinado tamanho o texto n„o È impresso, gerenado o erro de DEBUG/TOTVSAPI na DLL.
		...
		/*
			DIVISAO III ñ InformaÁıes de Total do DANFE NFC-e
		*/
		//--------------------------------------------
		// "Qtd. Total de Itens"
		//--------------------------------------------		
		...
		*/
		...
		/*
			Divis„o VI ñ InformaÁıes sobre o Consumidor
		*/
		cTexto += TAG_CENTER_INI
		...
		/*
			DIVISAO VI ñ InformaÁıes de IdentificaÁ„o da NFC-e e do Protocolo de AutorizaÁ„o
			N˙mero SÈrie Emiss„o DD/MM/AAAA hh:mm:ss
		*/
		aDtHrLocal := LjUTCtoLoc(aIdNfce:_DHEMI:TEXT)

		cTextoAux := "NFC-e n " + aIdNfce:_NNF:TEXT + " " 		//N˙mero da NFC-e
		cTextoAux += "SÈrie " + aIdNfce:_SERIE:TEXT + " " 		//SÈrie da NFC-e
		cTextoAux += PadL( Day(aDtHrLocal[1]),2,"0" ) + "/"		//DD
		cTextoAux += PadL( Month(aDtHrLocal[1]),2,"0" ) + "/"	//MM
		cTextoAux += cValToChar( Year(aDtHrLocal[1]) ) + " "	//AAAA
		cTextoAux += aDtHrLocal[2]								//hh:mm:ssa
		...
		If !lContigen			
			...
			//Data de AutorizaÁ„o: DD/MM/AAAA hh:mm:ss
			cTextoAux := (cTagCondIni + TAG_NEGRITO_INI + "Data de AutorizaÁ„o: " + TAG_NEGRITO_FIM + cTagCondFim)
			...
			cTexto += TAG_CENTER_INI 
			cTexto += (cTagCondIni + TAG_NEGRITO_INI + "Protocolo de AutorizaÁ„o: " + TAG_NEGRITO_FIM + cTagCondFim)
		...
		/*
			DIVISAO VIII ñ ¡rea de Mensagem Fiscal
		*/		
		If Type("oNfce:_NFE:_INFNFE:_INFADIC") == "O" .AND. LjRTemNode(oNfce:_NFE:_INFNFE:_INFADIC, "_INFADFISCO")
		...
		If cAmbiente == "2"
			cTexto += cCRLF
			cTexto += (TAG_CENTER_INI + TAG_CONDEN_INI + "EMITIDA EM AMBIENTE DE HOMOLOGA«√O ñ SEM VALOR FISCAL" + TAG_CONDEN_FIM + TAG_CENTER_FIM)
			cTexto += Replicate(cCRLF,2)
		EndIf
		...
		If lContigen			
			...
			cTexto += cCRLF
			cTexto += (TAG_CENTER_INI + cTagCondIni + "Pendente de autorizaÁ„o" + cTagCondFim + TAG_CENTER_FIM)
			cTexto += Replicate(cCRLF,2)
		EndIf
...
Verifica se existe o nÛ no XML
...
Else //se for hora fechada (ex: 08:00), o array somente vai ter uma posiÁ„o, sendo assim, adicionamos 00 aos Minutos
...
USER Function HFLjQrCo( oNFCe, cAmbiente ) //U_HFLjQrCo //LjNfceQrCo
	...
	Local cDest			:= ""								//oNfce:_NFE:_INFNFE:_DEST Documento de IdentificaÁ„o do Consumidor
